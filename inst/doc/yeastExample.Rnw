...
In \cite{geone} the authors consider an interesting question. They
assemble gene expression data from a yeast cell-cycle experiment
\citep{cho}, literature protein-protein interaction (PPI) data and
yeast two-hybrid data. We have curated the data slightly to make it
simpler to carry out the analyses reported in \cite{geone}. In
...
closely with the report in \cite{GePPI}.
